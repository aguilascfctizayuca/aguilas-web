...
export default function ActividadReciente() {
  ...
  useEffect(() => {
    async function cargar() {
      try {
        const q = query(collection(db, 'actividad_portal'), orderBy('timestamp', 'desc'), limit(8))
...
const styles = {
  box: {
    padding: '18px 20px', borderRadius: '14px', background: 'var(--portal-card-bg)',
    border: '1px solid var(--portal-card-border)', marginBottom: '24px',
  },
  titulo: { fontFamily: 'Montserrat, sans-serif', fontWeight: 900, fontSize: '15px', color: 'var(--portal-text)', margin: '0 0 12px' },
